...
public class Square extends Shape {
    ...
    public Square(float x1, float y1, float x2, float y2, int strokeColor, float width) {
        this(x1, y1, x2, y2, strokeColor, width, Color.TRANSPARENT);
    }

    public Square(float x1, float y1, float x2, float y2, int strokeColor, float strokeWidth, int fillColor) {
    ...
    @Override
    public Shape clone() {
        return new Square(p1.getX(), p1.getY(), p2.getX(), p2.getY(), this.getStrokeColor(), this.getStrokeWidth(), this.getFillColor());
    }
}
